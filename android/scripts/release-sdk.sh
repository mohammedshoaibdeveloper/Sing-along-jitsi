...
echo "Releasing Sing Along SDK ${SDK_VERSION}"
...
# Now build and publish the Sing Along SDK and its dependencies
echo "Building and publishing the Sing Along SDK"
pushd ${THIS_DIR}/../
...
if [[ $DO_GIT_TAG == 1 ]]; then
    ...
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Sing Along SDK + dependencies: ${SDK_VERSION}"
    popd
