...
class ConfirmDialog extends BaseSubmitDialog<Props, *> {
    ...
    _renderSubmittable() {
        ...
        console.log(_dialogStyles.text)
        ...
        return (
            <View>
                {this.props.waitingForHost && <Text
                style={{"color": "rgb(28, 32, 37)", "fontSize": 22, "textAlign": "center", top: -15, fontWeight: 'bold'}}
                >{"Waiting for host..."}</Text>}
                <Text style = { {"color": "rgb(28, 32, 37)", "fontSize": 16, "textAlign": "left", top: -10} }>
                    { content }
                </Text>
            </View>
        );
